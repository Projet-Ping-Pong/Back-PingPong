...
const gammeRepository = require('./gamme-repository');
const pieceRepository = require('../Piece/piece-repository');
const gammeoperationRepository = require('./gamme_operation-repository');
const utilisateurRepository = require('../../Admin/Utilisateur/utilisateur-repository')
...
router.post('/getId', validateJWT, async (req, res) => {
    try {
        if (req.body.id) {
            ...
            if(gamme.id_piece){
                piece = pieceRepository.getPieceById(gamme.id_piece)
            }
